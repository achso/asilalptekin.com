...
export default async function CaseStudyPage(props: PageProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-background text-black/85 flex flex-col items-center selection:bg-black selection:text-white">
      <Navbar />
      ...
      <header className="w-full max-w-5xl px-8 py-16 md:py-20 flex flex-col items-start relative z-10">
        ...
        <div className="flex flex-col gap-2 mb-12 max-w-3xl">
          <h1 className="text-4xl md:text-5xl lg:text-[64px] font-[800] tracking-tight text-black/85 leading-[1.1]">
            {project.title.split(':')[0]}
          </h1>
          <p className="text-xl md:text-[26px] text-black/30 font-[500] leading-[1.4] mt-4">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-[1fr_250px] gap-12 md:gap-24">
           ...
           <div className="flex flex-col gap-10">
              <div className="text-[15px] md:text-[16px] leading-[1.8] text-black/45 font-[500] max-w-2xl">
                 {project.overview ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-4 max-w-2xl">
                 <div className="flex flex-col gap-2">
                   <h3 className="font-[800] text-[15px] text-black/85">Problem</h3>
                   <p className="text-[14px] leading-[1.7] text-black/45 font-[500]">
                     {project.content?.problem || "Users faced significant friction managing tasks and data manually in disconnected systems."}
                   </p>
                 </div>
                 <div className="flex flex-col gap-2">
                   <h3 className="font-[800] text-[15px] text-black/85">Outcome</h3>
                   <p className="text-[14px] leading-[1.7] text-black/45 font-[500]">
          ...
          {project.sections && project.sections.map((section, index) => (
            <div key={index}>
              <section className="flex flex-col gap-6">
                <h2 className="text-2xl md:text-3xl font-[800] tracking-tight text-black/85">{section.title}</h2>
                <div className="text-[18px] md:text-[20px] text-black/45 leading-[1.8] space-y-6 font-[500]">
                  {Array.isArray(section.content) 
                    ? section.content.map((paragraph, i) => {
                        const parts = paragraph.split('**');
                        if (parts.length > 2) {
                           return <p key={i}><strong className="text-black/85 font-[800]">{parts[1]}</strong>{parts[2]}</p>
                        }
